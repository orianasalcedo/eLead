...
class CSVToCypressConverter {
  ...
  extractFeature(summary, description) {
    ...
    const featurePatterns = [
      ...
      {
        pattern: /page.*type|parent.*page|child.*page/i,
        feature: 'Page Management',
      },
      { pattern: /ghostcard|payment.*method/i, feature: 'Payment Methods' },
      {
        pattern: /stock|inventory|availability/i,
        feature: 'Inventory Management',
      },
      { pattern: /email|notification/i, feature: 'Email Notifications' },
      {
        pattern: /order.*confirmation|sale.*transaction/i,
...
`
  ...
  getFeatureDescription(feature) {
    const descriptions = {
      ...
      'Page Management':
        'Tests for parent/child page creation, editing, and management',
      'Payment Methods':
      ...
      'General Features':
        "General feature tests that don't fit into specific categories",
    }
...
if (require.main === module) {
  ...
  converter
    ...
    .then(() => {
      ...
      console.log(
        '4. Run tests with: npx cypress run --spec "cypress/e2e/features/**/*.cy.js"',
      )
    })
    .catch((error) => {
