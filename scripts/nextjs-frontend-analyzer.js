...
class NextjsFrontendAnalyzer {
  ...
  analyzeRepository() {
    ...
    try {
      ...
      analysis.frontendFeatures = this.analyzeFrontendFeatures(this.repositoryPath);
      console.log(`🛒 ${analysis.frontendFeatures.length} frontend features identified`);
    ...
    try {
      const findPages = (dir) => {
        ...
        for (const item of items) {
          ...
          if (stat.isDirectory()) {
            findPages(itemPath);
          } else if (item === 'page.tsx' || item === 'page.ts' || item === 'page.js' || item === 'page.jsx') {
            const content = fs.readFileSync(itemPath, 'utf8');
            pages.push({
              file: item,
              path: itemPath.replace(repoPath, ''),
              route: itemPath.replace(repoPath + '/src/app', '').replace('/page.tsx', '').replace('/page.ts', '').replace('/page.js', '').replace('/page.jsx', '') || '/',
  ...
  isFrontendPage(content, filename) {
    const frontendKeywords = [
      'cart', 'checkout', 'product', 'catalog', 'store', 'address',
      'payment', 'shipping', 'order', 'user', 'account', 'login'
    ];
        ...
        const findComponents = (dir) => {
          ...
          for (const item of items) {
            ...
            if (stat.isDirectory()) {
              ...
              if (item === 'page.tsx' || item === 'page.ts' || item === 'page.js' || item === 'page.jsx') {
                return;
              }
  ...
  isFrontendComponent(content, filename) {
    const frontendKeywords = [
      'cart', 'checkout', 'product', 'catalog', 'store', 'address',
      'payment', 'shipping', 'order', 'user', 'account', 'login',
      'button', 'form', 'input', 'dropdown', 'modal', 'card'
    ];
        ...
        for (const item of items) {
          ...
          if (stat.isDirectory()) {
            ...
            apiRoutes.push({
              file: item,
              path: itemPath.replace(repoPath, ''),
              hasGet: content.includes('export default function handler') || content.includes('GET'),
  ...
  generateReport(analysis) {
    ...
    // Save report
    const reportPath = path.join(__dirname, '..', 'docs', 'nextjs-frontend-analysis.json');
  ...
  generateRecommendations(analysis) {
    ...
    if (componentsWithTestIds < totalComponents * 0.3) {
      recommendations.push({
        type: 'selector',
        priority: 'high',
        message: 'Baja cobertura de data-testid en componentes. Considera agregar atributos data-testid para mejor estabilidad de tests.'
      });
    }
    ...
    if (analysis.testFiles.length === 0) {
      recommendations.push({
        type: 'testing',
        priority: 'high',
        message: 'No se encontraron archivos de test existentes. Considera implementar tests Jest junto con tests de Cypress.'
      });
    }
  ...
  async runAnalysis() {
    try {
      console.log('🚀 Iniciando análisis del repositorio Next.js eleadpromo-nextjs...\n');
      ...
      console.log(`🛠️  Encontrados ${report.summary.totalUtils} archivos de utilidades`);
      console.log(`📝 Encontrados ${report.summary.totalTypes} archivos de tipos`);
      ...
      console.log(`🛒 Encontradas ${report.summary.totalFrontendFeatures} características de frontend`);
...
if (require.main === module) {
  ...
  analyzer.runAnalysis()
    .then((report) => {
      ...
      console.log('📋 Revisa docs/nextjs-frontend-analysis.json para resultados detallados');
    })
    .catch(error => {
