...
class AdminRepositoryAnalyzer {
  ...
  async cloneRepository() {
    ...
    try {
      ...
      if (githubToken) {
        cloneUrl = this.repository.url.replace(
          'https://github.com/',
        ...
        console.log(
          '⚠️  No GITHUB_TOKEN found, attempting to clone without authentication',
        )
      }

      // Clone repository
      execSync(
        `git clone -b ${this.repository.branch} ${cloneUrl} ${this.repository.localPath}`,
        {
          stdio: 'inherit',
        },
      )
      ...
      console.log(
        `   git clone -b ${this.repository.branch} ${this.repository.url} ${this.repository.localPath}`,
      )
      return false
    }
  }
  ...
  analyzeRepository() {
    ...
    try {
      ...
      console.log(
        `🔌 ${analysis.apiEndpoints.length} archivos con endpoints API encontrados`,
      )

      // Find data-testid selectors
      analysis.selectors = this.findSelectors(this.repository.localPath)
      console.log(
        `🎯 ${analysis.selectors.length} archivos con data-testid encontrados`,
      )

      // Find existing test files
      analysis.testFiles = this.findTestFiles(this.repository.localPath)
      console.log(
        `🧪 ${analysis.testFiles.length} archivos de test encontrados`,
      )

      // Analyze admin-specific features
      analysis.adminFeatures = this.analyzeAdminFeatures(
        this.repository.localPath,
      )
      console.log(
        `⚙️  ${analysis.adminFeatures.length} características de admin identificadas`,
      )
    } catch (error) {
  ...
  generateReport(analysis) {
    ...
    const reportPath = path.join(
      ...
      '..',
      'docs',
      'admin-repository-analysis.json',
    )
  ...
  generateRecommendations(analysis) {
    ...
    if (componentsWithTestIds < totalComponents * 0.5) {
      recommendations.push({
        ...
        priority: 'high',
        message:
          'Baja cobertura de data-testid. Considera agregar atributos data-testid a los componentes para mejor estabilidad de tests.',
      })
    }
    ...
    if (analysis.testFiles.length === 0) {
      recommendations.push({
        ...
        priority: 'high',
        message:
          'No se encontraron archivos de test existentes. Considera implementar tests unitarios junto con tests de Cypress.',
      })
    }
  ...
  async runAnalysis() {
    try {
      console.log(
        '🚀 Iniciando análisis del repositorio e-lead-promo-admin...\n',
      )
      ...
      console.log(
        `🔌 Encontrados ${report.summary.totalApiEndpoints} archivos con endpoints API`,
      )
      console.log(
...
if (require.main === module) {
  ...
  analyzer
    ...
    .then((report) => {
      ...
      console.log(
        '📋 Revisa docs/admin-repository-analysis.json para resultados detallados',
      )
    })
    .catch((error) => {
