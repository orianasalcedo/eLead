...
class RepositoryAnalyzer {
  ...
  async cloneRepositories() {
    ...
    for (const [name, repo] of Object.entries(this.repositories)) {
      try {
        ...
        const authUrl = repo.url.replace('https://github.com/', `https://${githubToken}@github.com/`);
        ...
        console.log('💡 Try cloning manually or check your GitHub token permissions');
      }
    }
  }
  ...
  generateReport(analyses) {
    const report = {
      ...
      summary: {
        totalComponents: analyses.reduce((sum, analysis) => sum + analysis.components.length, 0),
        totalPages: analyses.reduce((sum, analysis) => sum + analysis.pages.length, 0),
        totalApiEndpoints: analyses.reduce((sum, analysis) => sum + analysis.apiEndpoints.length, 0),
        totalSelectors: analyses.reduce((sum, analysis) => sum + analysis.selectors.length, 0),
        totalTestFiles: analyses.reduce((sum, analysis) => sum + analysis.testFiles.length, 0)
      },
      recommendations: this.generateRecommendations(analyses)
    };

    // Save report
    const reportPath = path.join(__dirname, '..', 'docs', 'repository-analysis.json');
  ...
  generateRecommendations(analyses) {
    ...
    const totalComponents = analyses.reduce((sum, analysis) => sum + analysis.components.length, 0);
    const componentsWithTestIds = analyses.reduce((sum, analysis) => 
      sum + analysis.components.filter(comp => comp.hasDataTestId).length, 0);

    if (componentsWithTestIds < totalComponents * 0.5) {
      recommendations.push({
        type: 'selector',
        priority: 'high',
        message: 'Low data-testid coverage. Consider adding data-testid attributes to components for better test stability.'
      });
    }

    // Check for existing test files
    const totalTestFiles = analyses.reduce((sum, analysis) => sum + analysis.testFiles.length, 0);
    if (totalTestFiles === 0) {
      recommendations.push({
        type: 'testing',
        priority: 'high',
        message: 'No existing test files found. Consider implementing unit tests alongside Cypress tests.'
      });
    }

    // Check for API endpoint coverage
    const totalApiEndpoints = analyses.reduce((sum, analysis) => sum + analysis.apiEndpoints.length, 0);
    if (totalApiEndpoints > 0) {
      recommendations.push({
        type: 'api',
        priority: 'medium',
        message: 'API endpoints found. Consider implementing API tests in addition to UI tests.'
      });
    }
  ...
  async runAnalysis() {
    try {
      ...
      console.log(`🎯 Found ${report.summary.totalSelectors} data-testid selectors`);
