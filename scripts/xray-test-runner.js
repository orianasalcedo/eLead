...
class XrayTestRunner {
  ...
  extractTestId(title) {
    // Extract test case ID from test title
    const patterns = [/ELP-\d+/, /TEST-\d+/, /TC-\d+/]
  ...
  async uploadExecutionReport(executionReport) {
    const xrayUrl =
      this.config.xrayServerUrl || 'https://xray.cloud.getxray.app/api/v2'
    ...
    if (this.config.xrayClientId && this.config.xrayClientSecret) {
      ...
      const credentials = Buffer.from(
        `${this.config.xrayUsername}:${this.config.xrayPassword}`,
      ).toString('base64')
    ...
    if (!response.ok) {
      throw new Error(
        `Xray API error: ${response.status} ${response.statusText}`,
      )
    }
  ...
  async getXrayCloudToken() {
    ...
    if (!response.ok) {
      throw new Error(
        `Xray Cloud authentication failed: ${response.status} ${response.statusText}`,
      )
    }
