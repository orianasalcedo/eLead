...
describe('eLead Promo API - Order Management Tests', () => {
  ...
  beforeEach(() => {
    ...
    cy.log(
      `🧪 Running Order Management tests in ${environment.toUpperCase()} environment`,
    )
  ...
  describe('Order Listing and Viewing', () => {
    it('should get orders list', () => {
      cy.apiRequest('GET', '/api/v1/orders', null, {
        headers: authHeaders,
      }).then((response) => {
      ...
      cy.apiRequest('GET', '/api/v1/orders', null, { headers: authHeaders })
        .then((response) => {
          ...
          if (response.body.length > 0) {
            ...
            return cy.apiRequest('GET', `/api/v1/orders/${orderId}`, null, {
              headers: authHeaders,
            })
          } else {
    ...
    it('should handle non-existent order ID', () => {
      cy.apiRequest('GET', '/api/v1/orders/99999', null, {
        headers: authHeaders,
      }).then((response) => {
  ...
  describe('Order Creation', () => {
    it('should create a new order', () => {
      ...
      cy.apiRequest('POST', '/api/v1/orders', orderData, {
        headers: authHeaders,
      }).then((response) => {
        ...
        if (response.status === 200) {
          ...
          cy.log(
            `⚠️  Order creation returned ${response.status} - might need additional setup`,
          )
    ...
    it('should reject order with invalid product ID', () => {
      ...
      cy.apiRequest('POST', '/api/v1/orders', invalidOrderData, {
        headers: authHeaders,
      }).then((response) => {
    ...
    it('should reject order without required fields', () => {
      ...
      cy.apiRequest('POST', '/api/v1/orders', incompleteOrderData, {
        headers: authHeaders,
      }).then((response) => {
        expect(response.status).to.be.oneOf([400, 422])
        ...
        .then((response) => {
          if (response.body.message !== 'No orders found') {
            ...
            if (response.status === 200) {
              ...
              cy.log(
                `⚠️  Order status update returned ${response.status} - might require admin privileges`,
              )
  ...
  describe('Order Items and Personalization', () => {
    it('should handle order items with personalization', () => {
      ...
      cy.apiRequest('POST', '/api/v1/orders', personalizedOrderData, {
        headers: authHeaders,
      }).then((response) => {
    ...
    it('should validate personalization data', () => {
      ...
      cy.apiRequest('POST', '/api/v1/orders', invalidPersonalizationData, {
        headers: authHeaders,
      }).then((response) => {
        expect(response.status).to.be.oneOf([400, 422])
  ...
  describe('Order Pricing and Calculations', () => {
    it('should calculate order totals', () => {
      ...
      cy.apiRequest('POST', '/api/v1/orders/calculate', orderCalculationData, {
        headers: authHeaders,
      }).then((response) => {
        if (response.status === 200) {
          ...
          cy.log(
            `⚠️  Order calculation returned ${response.status} - endpoint might not exist`,
          )
  ...
  describe('Order History and Filtering', () => {
    it('should filter orders by status', () => {
      cy.apiRequest('GET', '/api/v1/orders?status=pending', null, {
        headers: authHeaders,
      }).then((response) => {
        expect(response.status).to.eq(200)
        expect(response.body).to.be.an('array')
    ...
    it('should paginate orders', () => {
      cy.apiRequest('GET', '/api/v1/orders?page=1&per_page=10', null, {
        headers: authHeaders,
      }).then((response) => {
        expect(response.status).to.eq(200)
        expect(response.body).to.be.an('array')
