...
describe('eLead Promo API - Contact Form Tests', () => {
  beforeEach(() => {
    ...
    cy.log(
      `🧪 Running Contact Form tests in ${environment.toUpperCase()} environment`,
    )
  })
  ...
  describe('Contact Us Requests', () => {
    it('should create contact us request with valid data', () => {
      ...
      cy.apiRequest('POST', '/api/v1/contact_us_requests', contactData).then(
        (response) => {
    ...
    it('should handle long messages appropriately', () => {
      ...
      cy.apiRequest(
        ...
        if (response.status === 200 || response.status === 201) {
          ...
          cy.log(
            `⚠️  Long message returned ${response.status} - might have length limits`,
          )
  ...
  describe('Contact Form Validation', () => {
    ...
    it('should validate name length', () => {
      ...
      cy.apiRequest('POST', '/api/v1/contact_us_requests', longNameData).then(
        (response) => {
          if (response.status === 200 || response.status === 201) {
            cy.log('✅ Long name accepted')
          } else {
            cy.log(
              `⚠️  Long name returned ${response.status} - might have length limits`,
            )
            expect(response.status).to.be.oneOf([200, 201, 400, 422])
          }
        },
    ...
    it('should validate subject length', () => {
      ...
      cy.apiRequest(
        ...
        if (response.status === 200 || response.status === 201) {
          ...
          cy.log(
            `⚠️  Long subject returned ${response.status} - might have length limits`,
          )
          expect(response.status).to.be.oneOf([200, 201, 400, 422])
        }
      })
    ...
    it('should handle HTML content in message', () => {
      const htmlData = {
        contact_us_request: {
          ...
          email: 'test@example.com',
          message:
            'Test message with <b>HTML</b> content and <script>alert("test")</script>',
        },
      }

      cy.apiRequest('POST', '/api/v1/contact_us_requests', htmlData).then(
        (response) => {
          expect(response.status).to.be.oneOf([200, 201])
    ...
    it('should handle SQL injection attempts', () => {
      const sqlInjectionData = {
        contact_us_request: {
          ...
          email: 'test@example.com',
          message:
            "Test message with SQL injection: '; DROP TABLE contacts; --",
        },
      }
  ...
  describe('Contact Form Performance', () => {
    ...
    it('should handle multiple simultaneous submissions', () => {
      ...
      cy.apiRequest('POST', '/api/v1/contact_us_requests', contactData1).then(
        (response1) => {
      ...
      cy.apiRequest('POST', '/api/v1/contact_us_requests', contactData2).then(
        (response2) => {
      ...
      cy.apiRequest('POST', '/api/v1/contact_us_requests', contactData).then(
        (response) => {
          ...
          const createdAt = new Date(
            response.body.contact_us_request.created_at,
          )
          const now = new Date()
