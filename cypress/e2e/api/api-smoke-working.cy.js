...
describe('eLead Promo API Smoke Tests - Working', () => {
  beforeEach(() => {
    ...
    cy.log(
      `🧪 Running API smoke tests in ${environment.toUpperCase()} environment`,
    )
  })

  it('should access store settings endpoint', () => {
    cy.apiRequest('GET', '/api/v1/store_settings').then((response) => {
  ...
  it('should access home settings endpoint', () => {
    cy.apiRequest('GET', '/api/v1/home_settings').then((response) => {
      expect(response.status).to.eq(200)
  ...
  it('should test authenticated endpoints after login', () => {
    ...
    cy.apiRequest('POST', '/api/v1/customers/sign_in', signinData)
      .then((response) => {
        ...
        return cy.apiRequest('GET', '/api/v1/countries', null, {
          headers: authHeaders,
        })
      })
      .then((response) => {
        ...
        cy.log(
          `Countries endpoint response body: ${JSON.stringify(response.body)}`,
        )
        ...
        if (response.status === 200) {
          ...
          cy.log(
            `⚠️  Countries endpoint returned ${response.status} - might need different auth approach`,
          )
  ...
  it('should test payment methods endpoint', () => {
    cy.apiRequest('GET', '/api/v1/payment_methods').then((response) => {
      ...
      if (response.status === 200) {
        ...
        cy.log(
          `⚠️  Payment methods endpoint returned ${response.status} - might require authentication`,
        )
  ...
  it('should test shipping methods endpoint', () => {
    cy.apiRequest('GET', '/api/v1/shipping_methods').then((response) => {
      ...
      if (response.status === 200) {
        ...
        cy.log(
          `⚠️  Shipping methods endpoint returned ${response.status} - might not exist or require authentication`,
        )
        expect(response.status).to.be.oneOf([200, 401, 404]) // Accept various status codes
      }
    })
  ...
  it('should test categories endpoint', () => {
    cy.apiRequest('GET', '/api/v1/categories').then((response) => {
      ...
      if (response.status === 200) {
        ...
        cy.log(
          `⚠️  Categories endpoint returned ${response.status} - might require authentication`,
        )
